...
if($_REQUEST['a'] == '')
{
  ...
  echo '<form name="form" action="'.$modulelink.'&a=settings" method="POST" class="form-horizontal">
          <div class="panel panel-default">
            ...
            <div class="form-group">
              <label class="col-md-3 control-label">'.$LANG['enabletos'].'</label>
              <div class="col-md-6">
                <div class="checkbox">
                  <label>
                    <input type="checkbox" name="tos" '; if ($CONFIG['TermsOfService'] && $CONFIG['EnableTOSAccept']){ echo'CHECKED'; } echo '>'.$LANG['enabletoshelp'].'
                  </label>
                </div>
              </div>
            </div>            
  ...
  echo '<p><a class="btn btn-primary" href="'.$modulelink.'&a=manage" role="button"><i class="fa fa-plus"></i> '.$LANG['createnewterms'].'</a>
  <a class="btn btn-success" href="'.$CONFIG['SystemURL'].'/index.php?m=TermsOfService" role="button" target="_blank"><i class="fa fa-globe"></i> '.$LANG['termsofservicepage'].'</a></p>';
  echo '<div class="panel panel-default">
          <table class="table table-striped table-hover">
            ...
            <tbody>';
  $result = Capsule::table('mod_termsofservice')->where('language', '')->orderBy('orders', 'ASC')->get();
...
if($_REQUEST['a'] == 'manage')
{
  ...
  if($id)
  {
    ...
    $data = Capsule::table('mod_termsofservice')->where('language', '')->where('id', $id)->first();
    ...
    $result = Capsule::table('mod_termsofservice')->where('parentid', $id)->get();
  ...
  if($id){echo '<h1><i class="fa fa-pencil-square-o"></i> '.$LANG['editterms'].'</h1>';}else{echo '<h1><i class="fa fa-pencil-square-o"></i> '.$LANG['createnewterms'].'</h1>';}
...
if($_REQUEST['a'] == 'save')
{
  ...
  if($id)
  {
    Capsule::table('mod_termsofservice')->where('id', $id)->update([
                                                                'title'    => $title, 
                                                                'contents' => $message, 
                                                                'status'   => $disable, 
                                                                'orders'   => $sorts, 
  ...
  {
    $id = Capsule::table('mod_termsofservice')->insertGetId([
                                                                'title'    => $title, 
                                                                'contents' => $message, 
                                                                'status'   => $disable, 
                                                                'orders'   => $sorts, 
  ...
  {
    Capsule::table('mod_termsofservice')->where('parentid', $id)->where('language', $language)->delete();
  
    if($title)
    {
      Capsule::table('mod_termsofservice')->insert([
                                                              'parentid' => $id, 
...
{
  Capsule::table('mod_tosconfog')->where('setting', 'Description')->update(['value' => $_REQUEST['description']]);
  Capsule::table('mod_tosconfog')->where('setting', 'Keyword')->update(['value' => $_REQUEST['keywords']]);

  if($_REQUEST['tos'])
  {
    Capsule::table('tblconfiguration')->where('setting', 'TermsOfService')->update(['value' => 'index.php?m=TermsOfService']);
    Capsule::table('tblconfiguration')->where('setting', 'EnableTOSAccept')->update(['value' => 'on']);
  } 
...
if($_REQUEST['a'] == 'delete')
{
  ...
  Capsule::table('mod_termsofservice')->where('id', $id)->delete();
  Capsule::table('mod_termsofservice')->where('parentid', $id)->delete();
